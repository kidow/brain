...
> **CRITICAL** — `notes/`, `sheets/` 또는 `drills/`에 파일을 추가·삭제한 응답에서 반드시 같은 응답 안에 README 두 파일을 업데이트한다. 커밋 전 체크리스트:
> 1. `README.md` 해당 섹션에 링크 추가/삭제했는가?
> 2. `README.ko.md` 해당 섹션에 링크 추가/삭제했는가?
> 3. 두 파일 모두 같은 커밋에 포함했는가?

`notes/`, `sheets/` 또는 `drills/` 폴더에 파일이 추가되거나 삭제되면 **반드시** `README.md`와 `README.ko.md` 양쪽의 해당 섹션을 동기화한다.
