...
## 군사 유래 관용구(Military Idioms)

전쟁·군대 문화가 일상어로 확장된 표현.

```
call the shots            결정권 쥐다, 지휘하다
under fire                 비난받는 중, 공격받는 중
go AWOL                    허락 없이 사라지다(무단결근)
hold the fort               자리 지키다, 대신 맡다
bite the bullet             (고통스러운 일) 각오하고 감수하다
on the front line           최전선에서, 가장 직접 부딪히는 위치
with flying colors          크게 성공해서
```

예문:
```
"She calls the shots around here, not the manager."
"The CEO has been under fire since the layoffs."
"Just bite the bullet and tell him the truth."
```

어원:
- call the shots — 옛 포병이 발사 시점·목표 지시하던 것
- bite the bullet — 마취 없던 시절 수술 시 총알 물고 고통 참던 것
- with flying colors — 승전 함선이 깃발(colors) 올린 채 당당히 귀항하던 모습

**핵심 이미지:** 군사 유래 = "위기·결단·권한" 프레임. 카드 유래(전략적 속임수)·항해 유래(상태 묘사)와 다른 결.
