...
- **place-me** — 학습을 시작하기 전, 해당 주제를 얼마나 알고 있는지 짧은 적응형 진단으로 가늠합니다. 한 번에 한 질문씩, 수준이 확인될 때까지 묻고, 결과를 서술형 수준 요약으로 `levels.md`에 기록합니다.
- **teach-me** — 한 번에 지식 하나씩, 기초부터 심화 순으로 설명합니다. "다음"이라고 할 때만 진행하고, 이미 배운 내용은 다시 가르치지 않습니다. 복잡한 개념은 모국어·문화에 빗대 설명하고, 이해가 어려우면 시각 자료를 제공합니다. 외국어 학습 시 발음이 필요하면 음성 자료도 재생합니다. 설명한 지식은 `notes/topic_name.md`에 커밋합니다.
- **review-me** — 저장된 노트를 한 항목씩 퀴즈로 복습합니다. 간격 반복(spaced-repetition) 방식이며, 컨텍스트가 없으면 `levels.md`에서 수준이 낮은 주제부터 우선 복습합니다. 복습이 끝나면 그 결과를 토대로 `levels.md`의 수준을 갱신합니다.
